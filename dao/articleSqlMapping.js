let sql = {
	...
	deleteArticleCatRel: "delete from k_article_category_relationship where a_id in (?)",
	...
	insertArticle: "insert into k_article (a_title,a_content,a_markdown,a_type,u_id,a_modifier_id,cat_id,a_updatedate,a_createdate) values(?,?,?,?,?,?,?,?,?)",
	updateArticle: "update k_article set a_title = ?,a_content = ?,a_markdown = ?,a_type = ?,u_id = ?,a_modifier_id = ?,cat_id = ?,a_updatedate = ? where a_id=?",
	insertArticleCatRel: "insert into k_article_category_relationship (a_id,cat_id) values(?,?)",
	queryArticles: (filters) => `SELECT a_id AS articleId, a_title AS title, a_status AS status, a_type AS articleType, a_createdate AS createDate , a_updatedate AS updateDate, a_banner AS banner, a_viewcount AS viewCount, a_likecount AS likeCount, a_reject_remark AS rejectRemark , a_modifier_id AS modifierId, d.u_name AS modifierName, b.u_name AS userName, b.u_id AS userId, c.cat_name AS categoryName , c.cat_id AS categoryId FROM k_article a LEFT JOIN k_user b ON a.u_id = b.u_id LEFT JOIN k_category c ON a.cat_id = c.cat_id LEFT JOIN k_user d ON a.a_modifier_id = d.u_id  where ${filters} limit ?,?`,
