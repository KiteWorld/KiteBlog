let sql = {
	...
	//查询
	queryLevelCat: "SELECT cat_name categoryName,cat_id categoryId FROM k_category WHERE cat_type = ? AND cat_level = 1",
	//查询该分类之下的所有子分类的内容总数（包括当前分类的数量）
	queryArticleCatTotal: `SELECT SUM(total) AS total FROM ( SELECT cat1.cat_name AS categoryName, count(acr_id) AS total FROM ( SELECT cat_id, cat_name FROM ( SELECT t1.cat_id, t1.cat_name , IF(FIND_IN_SET(cat_parentid, @pids) OR FIND_IN_SET(cat_id, @pids) > 0, @pids := CONCAT(@pids, ',', cat_id), 0) AS ischild FROM ( SELECT cat_id, cat_parentid, cat_name FROM k_category ORDER BY cat_parentid, cat_id ) t1, ( SELECT @pids := ? ) t2 ) t3 WHERE ischild != 0 ) cat1 LEFT JOIN k_article_category_relationship ON cat1.cat_id = k_article_category_relationship.cat_id GROUP BY cat1.cat_id ) totaltable`,
	queryHotPointCatTotal: `SELECT SUM(total) AS total FROM ( SELECT cat1.cat_name AS categoryName, count(hpc_id) AS total FROM ( SELECT cat_id, cat_name FROM ( SELECT t1.cat_id, t1.cat_name , IF(FIND_IN_SET(cat_parentid, @pids) OR FIND_IN_SET(cat_id, @pids) > 0, @pids := CONCAT(@pids, ',', cat_id), 0) AS ischild FROM ( SELECT cat_id, cat_parentid, cat_name FROM k_category ORDER BY cat_parentid, cat_id ) t1, ( SELECT @pids := ? ) t2 ) t3 WHERE ischild != 0 ) cat1 LEFT JOIN k_hp_category_relationship ON cat1.cat_id = k_hp_category_relationship.cat_id GROUP BY cat1.cat_id ) totaltable`,
	//查询该分类之下的所有子分类的内容总数(包括当前分类的数量)
