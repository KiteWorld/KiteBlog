var sql = {
	//ToC用户
	insert: "insert into k_user(u_name,u_password,u_role,u_status,u_avatar,u_sex,u_createdate) VALUES(?,?,?,?,?,?,?)",
	update: "update k_user set u_name=?,u_password=?,u_role=? ,u_status=? ,u_avatar=? ,u_sex=? ,u_createdate=? where u_id=?",
	...
	queryUserById: "select u_id userId,u_name userName,u_password password,u_role userRole,u_sex userSex,u_avatar avatar,u_status userStatus from k_user where u_id=?",
	queryByName: "select * from k_user where u_name=?",
	queryUser: (filters) => `select u_id as userId, u_name as userName,u_role as userRole,u_status as userStatus,u_sex as userSex ,u_avatar as avatar ,u_createdate as createDate from k_user where ${filters} limit ?,?`,
	...
	//CMS用户
	queryCMSUserById: "select cms_u_id as CMSUserId, cms_u_name as userName,cms_u_password as password,cms_u_role as userRole,cms_u_avatar as avatar,a.u_id ToCUserId,cms_u_createdate as createDate ,b.u_name ToCUserName from k_cms_user a left join k_user b on a.u_id = b.u_id where cms_u_id=?",
